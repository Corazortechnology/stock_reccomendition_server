...
const register_mail = async (email, userName, otp) => {
  ...
  const mailOption = {
    ...
    html: `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
        <div style="margin:50px auto;width:70%;padding:20px 0">
          ...
          <p>Thank you for choosing Stock. Use the following OTP to complete your Sign Up procedures. OTP is valid for 2 minutes</p>
          <h2 style="background: #00466a;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;">${otp}</h2>
          <p style="font-size:0.9em;">Regards,<br />Stock</p>
...
const reset_password_mail = async (email, userName, otp) => {
  // email sent for verification
  //1 transfer protocal
  const transporter = nodemailer.createTransport({
    service: "gmail",
...
const register_greet = async (userName, email) => {
  // email sent for verification
  //1 transfer protocal
  const transporter = nodemailer.createTransport({
    service: "gmail",
...
const verify_greet = async (userName, email) => {
  // email sent for verification
  //1 transfer protocal
  const transporter = nodemailer.createTransport({
    service: "gmail",
...
const password_updated = async (userName, email) => {
  // email sent for verification
  //1 transfer protocal
  const transporter = nodemailer.createTransport({
    service: "gmail",
