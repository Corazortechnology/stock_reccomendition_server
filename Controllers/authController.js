  ...
  if (userName == "" || email == "" || password == "") {
    ...
    if (isUser.length) {
      if (!isUser[0].is_varified) {
        let hasOtp = await ownerEmailverification.find({
          owner: isUser[0]._id,
        });
        ...
        if (hasOtp.length) {
          if (hasOtp[0].date_expires < Date.now()) {
            ...
            const ans = await ownerEmailverification.findOneAndDelete({
              _id: hasOtp[0]._id,
            });
            ...
            //creating otp
            const otp = String(
              (parseInt(uuidv4().slice(0, 7), 16) % 900000) + 100000
            );
            //hash otp
            let salt = 10;
            ...
            // sending email
            const result = await register_mail(
              isUser[0].email,
              isUser[0].userName,
              otp
            );
          ...
          //generate otp
          const otp = String(
            (parseInt(uuidv4().slice(0, 6), 16) % 900000) + 100000
          );
          //hash otp
          let salt = 10;
          let hashedOtp = await bcrypt.hash(otp, salt);
          ...
          if (result) {
            ...
            res.json({
              ...
              message:
                "users already present but email is not varified. email is sent to your given eamil",
            });
          } else {
            res.json({
              status: "error",
      ...
      if (result) {
        ...
        await register_greet(userName, email);
    ...
    if (isUser.length) {
      if (isUser[0].date_expires < Date.now()) {
        ...
        if (same) {
          ...
          await ownerEmailverification.findOneAndDelete({
            owner: tokenData.user,
          });
